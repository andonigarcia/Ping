...
```
  ...
  * **/mobile/api/v0.1/users/<int:id>/map/<int:companyid>** - GET
    ...
    On success, it returns a status code of 200, with a JSON response of: `{'deal': DEAL}`, a DEAL consists of: `{'company_id': ID, 'logo': URI, 'address': ADDRESS, 'deal': DEAL, 'latlong': [latitude, longitude], 'phone': phone}`  
    On failure, it returns either a status code 400 or a status code 403 depending on the failure.

```

	$ curl -u eyJhbGciOiJIUzI1NiIsImV4cCI6MTQyNTg5NzU2OCwiaWF0IjoxNDI1NjM4MzY4fQ.eyJpZCI6MX0.oMHHi1_7qFuOgQiYD6nhsOMRGNWHCX2E8shP1EVQcyI:token -i -X GET http://localhost:5000/mobile/api/v0.1/users/1/map/1
	HTTP/1.0 200 OK
	Content-Type: application/json
	Content-Length: 18
	Server: Werkzeug/0.10 Python/2.7.5
	Date: Fri, 06 Mar 2015 10:47:10 GMT
